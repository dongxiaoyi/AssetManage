...
from .models import OperateRecord,ReturnRecord

class OperateRecordAdminx(object):
    list_display = ['nowtime','username','user_operate','simple_tgt','jid']
    #搜索框
    search_fields = ['username','user_operate','simple_tgt','jid']
    #过滤器
    list_filter = ['nowtime','username','user_operate','simple_tgt','jid']
    ordering = ['-nowtime']
    #list_editable = ['name']
